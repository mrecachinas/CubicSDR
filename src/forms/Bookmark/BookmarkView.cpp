...
public:
    explicit ActionDialogRemoveBookmark( BookmarkEntryPtr be ) : ActionDialog(wxGetApp().getAppFrame(), wxID_ANY, wxT("Remove Bookmark?")) {
        subject = be;
        m_questionText->SetLabelText(wxString("Are you sure you want to remove the bookmark\n '") + BookmarkMgr::getBookmarkEntryDisplayName(subject) + wxString("'?"));
    }
...
public:
    explicit ActionDialogRemoveGroup( std::string groupName ) : ActionDialog(wxGetApp().getAppFrame(), wxID_ANY, wxT("Remove Group?")) {
        subject = groupName;
        m_questionText->SetLabelText(wxString("Warning: Are you sure you want to remove the group\n '") + subject + wxString("' AND ALL BOOKMARKS WITHIN IT?"));
    }
